...
def sendVideo():
    ...
    if bot.get_updates():
        chat_id =  '-790881518' 
        video=open(f"D:\\python_idleprog\\Practice\\output{i}.avi", 'rb')
        now = datetime.now()
...
def cameraFunction():
    ...
    out = cv2.VideoWriter(f"D:\\python_idleprog\\Practice\\output{i}.avi",cv2.VideoWriter_fourcc('M','J','P','G'), 10, (frame_width,frame_height))
